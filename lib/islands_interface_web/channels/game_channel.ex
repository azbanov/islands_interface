defmodule IslandsInterfaceWeb.GameChannel do
  ...
  def handle_in("set_islands", player, socket) do
    ...
    case Game.set_islands(via(socket.topic), player) do
      {:ok, board} -> broadcast! socket, "player_set_islands", %{player: player}
        # {:ok, enc_board} = Poison.encode(board)
        # {:ok, dec_board} = Poison.decode(enc_board)
        {:reply, {:ok, %{board: encode!(board) |> decode!()}}, socket}
      _ -> {:reply, :error, socket}
    end
  end
